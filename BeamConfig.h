...
void DataReceiveCallBack(EthernetClient &stream)
{
	...
	{
		memccpy(httpHeader.code, httpHeader.buf, 0, receivedBytes);
		log_ln(httpHeader.code);

		if (strstr(httpHeader.code, "OK") != NULL)
		{
			log_ln("Request successfull");
			log_ln(httpHeader.buf);
		}
		else
		{
			if (strstr(httpHeader.code, "Not Found") != NULL)
			{
				log_ln("Request failed");
				RootNextion.GotoPage(BKanban.CurrentWindowId);
				return;
			}
			else
